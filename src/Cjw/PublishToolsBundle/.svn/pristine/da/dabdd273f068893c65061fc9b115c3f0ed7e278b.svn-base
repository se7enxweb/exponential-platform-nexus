...
use Cjw\PublishToolsBundle\Form\Type\FormBuilderType;
/*
...
class FormBuilderService
{
    ...
    {
        $formBuilder = $this->container->get('form.factory')->createBuilder( FormBuilderType::class );
...
//            $formBuilder->add( $key, $type, $formAttrArr );
            $formBuilder->add( $key, $this->matchTypeClass( $type ), $formAttrArr );
        }
        ...
        if( $parameters ) {
            if( isset( $parameters['button_config']['save_button']['label'] ) ) {
                ...
                $hasLanguageCode = isset( $parameters['button_config']['save_button']['label'][$languageCode] );
        ...
        $formBuilder->add( 'save', $this->matchTypeClass( 'submit' ), array( 'label' => $labelSaveButton, 'attr' => array('class' => 'btn-primary') ) );

        if( isset( $parameters['button_config']['cancel_button'] ) && $parameters['button_config']['cancel_button'] !== false ) {
            $formBuilder->add( 'cancel', $this->matchTypeClass( 'submit' ), array( 'label' => $labelCancelButton, 'attr' => array( 'formnovalidate' => '' ) ) );
        }
...
//                            $fieldArr['value'] = $this->ezXmltextToHtml( $content, $field );
                        ...
                        case 'ezdate':
                            $formFieldIdentifier = 'ezdate' . FormHandlerService::separator . $field->identifier;
                            $fieldArr['type'] = 'date';
                            $fieldArr['value'] = $field->defaultValue->date;
                            break;

                        case 'ezboolean':
                            $formFieldIdentifier = 'ezboolean' . FormHandlerService::separator . $field->identifier;
...
//                                $fieldArr['choices'][$fieldChoiceKey] = $this->container->get( 'translator' )->trans( $fieldChoice );
                                $fieldArr['choices'][$this->container->get( 'translator' )->trans( $fieldChoice )] = $fieldChoiceKey;
                            }
                    ...
                    if ( $content )
                    {
                        $formFieldIdentifierArr = explode( FormHandlerService::separator, $formFieldIdentifier );
...
// ToDo: besser lösen, ist folgendes kompatible mit plain ezp5, was ist wenn tools bundle in vendor dir ?
    ...
    // http://share.ez.no/forums/ez-publish-5-platform/papi-convert-xmltext-field
    private function matchTypeClass( $type )
    {
        $typeClass = null;

        switch( $type ) {
            case 'choice':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\ChoiceType::class;
                break;

            case 'checkbox':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\CheckboxType::class;
                break;

            case 'radio':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\RadioType::class;
                break;
            ...
            case 'email':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\EmailType::class;
                break;

            case 'date':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\DateType::class;
                break;

            case 'datetime':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\DateTimeType::class;
                break;

            case 'time':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\TimeType::class;
                break;

            case 'password':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\PasswordType::class;
                break;
            ...
            case 'integer':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\IntegerType::class;
                break;

            case 'number':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\NumberType::class;
                break;

            case 'tel':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\TelType::class;
                break;

            case 'file':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\FileType::class;
                break;

            case 'hidden':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\HiddenType::class;
                break;

            case 'button':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\ButtonType::class;
                break;

            case 'submit':
                $typeClass = \Symfony\Component\Form\Extension\Core\Type\SubmitType::class;
                break;
