...
//        foreach( $params['form']->vars['value'] as $contentTypeIdentifier => $dataType ) {
        foreach( $params['form']->children as $contentTypeIdentifier => $dataType ) {
            $FullContentTypeName = explode(FormHandlerService::separator,$contentTypeIdentifier);
            if( isset( $FullContentTypeName[1] ) ) {
                //e.g ezstring
                ...
                $ezDataType = $FullContentTypeName[0];
                //e.g lastname
                $identifierName = $FullContentTypeName[0];
            }
...
// ToDo: catch "Argument 'userCreateStruct' is invalid: User with provided login already exists" (InvalidArgumentException) exception
    ...
    private function processHandlers( $configIdentifierStr, $formDataObj, $handlerParameters, $form )
    {
        ...
        // check if validateFields handler is set in 'handler' block ( which is located in formbuilder.yml )
        if(isset($handlerConfigArr['validateFields'])) {
            $fieldToValidateHasValue = false;

            // todo: make it work with multiple fields
            foreach($handlerConfigArr['validateFields'] as $fieldIdToValidate) {
                // check if field to validate has value
                if(isset($formDataObj[$fieldIdToValidate])) {
                    if(($formDataObj[$fieldIdToValidate] !== null) || ($formDataObj[$fieldIdToValidate] !== '')) {
                        $fieldToValidateHasValue = true;
                    }
                }
            }

            // if field to validate has no value, send success message.
            if(($fieldToValidateHasValue === false)) {
                $response = $this->formHandlerService->successHandler( $formDataObj, $handlerConfigArr['success'], $handlerParameters, $form );
        ...
        foreach( $handlerConfigArr as $handlerConfig ) {
            ...
            if( isset( $handlerConfig['handler_class'] ) ) {
                ...
                // Check whether the given $handler_class method exists in the "formHandlerService" object member.
...
//        $this->get('request')->getSession()->invalidate();
    ...
    private function userRegisterHandler( $formDataObj, $locationId, $contentType )
    {
        ...
        // the id from config, from current user to admin user
