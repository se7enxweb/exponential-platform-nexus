...
// todo check valid syntax
        ...
        if( isset( $params['filter_by_time'] ))
        {
            // filtert Objekte die [VOR oder NACH] einem [ZEITPUNKT] [CREATED oder MODIFIED] wurden
            // Example 1)
            // calculate 'time_period' in seconds !!!
            // 'filter_by_time': {
            //       'date_type': 'created'  ,
            //       'temporal_direction': 'after'  ,
            //       'time_period' : onYearBefore ,
            //        },
            //
            // Example 2)
            // shortest with all default values
            // 'filter_by_time' : true,
            $dateType = Criterion\DateMetadata::CREATED;// default date_type "creation" ( Das Filterkriterum )
            if( isset( $params['filter_by_time']['date_type'] )
                && $params['filter_by_time']['date_type'] == 'modified')
            {
                $dateType = Criterion\DateMetadata::MODIFIED;
            }

            $temporalDirection = Operator::GTE;// default direction "after" the giver time peroid-> From Past to Future ( Die Richtung des Zeitraumes. VOR oder NACH dem Zeitpunkt! )
            if( isset( $params['filter_by_time']['temporal_direction'] )
                && $params['filter_by_time']['temporal_direction'] == 'before')
            {
                $temporalDirection = Operator::LTE;
            }
            //NEU 150 Tage
            $timePeriod = time() - (24 * 3600 * 150 ) ;// default 1 year before now() ( Der STARTPUNKT für den Zeitraum, von dem aus gefiltert wird )
            if( isset( $params['filter_by_time']['time_period'] )
                && $params['filter_by_time']['time_period'] > 0 )
            {
                $timePeriod = time() - $params['filter_by_time']['time_period'];
            }

            $criterion[] =  new Criterion\DateMetadata( $dateType, $temporalDirection, $timePeriod );
...
//                    $this->logger->info( "ContentType '$classIdentifier' not exists, remove it from your include array in cjw_fetch!", [ __METHOD__ => $classIdentfierArray ] );
    ...
    public function getTags( $parentTagId = null )
    {
        ...
        if ( $parentTagId !== NULL )
        {
            $parentTag = $this->tagsService->loadTag( (int) $parentTagId );
        }

        $listTags = $this->tagsService->loadTagChildren( $parentTag );

        $listTagsReturn = array();

        if ( isset( $listTags[ 0 ] ) )
        {
            foreach ( $listTags as $tag )
            {
                $relatedContentCount = (int) $this->tagsService->getRelatedContentCount( $tag );

                $listTagsReturn[] = array( 'tag' => $tag, 'count_related_content' => $relatedContentCount );
            }
        }

        return $listTagsReturn;
    }
}
