...
class TwigContentFunctionsService extends Twig_Extension
{
    ...
    public function myCustomTemplate(\Twig_Environment $environment)
    {
        return $environment->render('CjwPublishToolsBundle::line.html.twig');
    }
     ...
     *
     * => If you want to render a location, which is invisible, you can use
     *    the 'ignore_visibility' parameter.
     *
     * ```twig
     * {# === RENDER === #}
     * {{ cjw_render_location( {
     ...
     * @return string
     */
    public function renderLocation( \Twig_Environment $environment, array $params )
    {
        ...
        elseif( isset( $params['locationId'] ) )
        {
            ...
            $ignoreVisibility = (bool) (isset( $params['ignore_visibility'] )) ? $params['ignore_visibility'] : false;

            $location = $this->repository->getLocationService()->loadLocation( $locationId );
            if ( !$ignoreVisibility && $location->invisible )
            {
        ...
        else $template = $this->getOverrideTemplate( $locationObject, $contentObject, $viewType );
     ...
     //   return $environment->render('CjwPublishToolsBundle::line_controller.html.twig');
...
//        $locationViewArrayAlt = $this->configResolver->getParameter( 'location_view' );
//
//        var_dump( $locationViewArrayAlt );

        // content_view ersetzt anscheinend locationview in ezplatform - content_view rendert block content_view zuerst und dann den alten location_view block
        $locationViewArray = $contentViewArray = $this->configResolver->getParameter( 'content_view' );

//        var_dump( $contentViewArray );
...
//    }
    ...
    public function getContentTypeIdentifier( $contentTypeId )
    {
     ...
     //   $contentTypeList = $cs->loadContentTypeList();
        ...
        try
        {
            ...
            // get a list of all contentTypeIdentifiers
//            $contentTypeIdentifierIdArray = [];
//            $contentTypeGroups = $cs->loadContentTypeGroups();
//            foreach( $contentTypeGroups as $group )
//            {
//                $contentTypes = $cs->loadContentTypes( $group );
//                foreach ( $contentTypes as $type )
//                {
//                    $contentTypeIdentifierIdArray[ $type->identifier ] = $type->id;
//                }
//            }
//            var_dump( $contentTypeIdentifierIdArray );
//            var_dump( $contentTypeGroups );
